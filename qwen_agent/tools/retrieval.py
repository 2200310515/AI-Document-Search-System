...
class Retrieval(BaseTool):
    ...
    def __init__(self, cfg: Optional[Dict] = None):
        ...
        if len(self.rag_searchers) == 1:
            # 传递完整的配置，包括Elasticsearch配置
            search_cfg = {'max_ref_token': self.max_ref_token}
            # 传递特定搜索器的配置
            searcher_name = self.rag_searchers[0]
            if searcher_name in self.cfg:
                search_cfg.update(self.cfg[searcher_name])
            self.search = TOOL_REGISTRY[searcher_name](search_cfg)
        else:
            from qwen_agent.tools.search_tools.hybrid_search import HybridSearch
            hybrid_cfg = {'max_ref_token': self.max_ref_token, 'rag_searchers': self.rag_searchers}
            # 传递各个搜索器的配置
            for searcher in self.rag_searchers:
                if searcher in self.cfg:
                    hybrid_cfg[searcher] = self.cfg[searcher]
            self.search = HybridSearch(hybrid_cfg)
